...
#-----------------------------------------------------#
def preprocessing(architecture, ensembler, config):
    # Get path to phase for correct dataset
    path_phase = os.path.join(config["path_results"], "phase_bagging" + "." + \
                              config["seed"], architecture)
    # Load ground truth for testing set
    path_gt = os.path.join(path_phase, "inference.test.set_y.csv")
    gt_map = pandas.read_csv(path_gt, header=0, index_col="index").to_dict()
    gt_map = gt_map["Ground_Truth"]

    # Create Inference IO Interface
    path_inf = os.path.join(path_phase, "inference", "inference." + ensembler + ".pred.json")
...
#-----------------------------------------------------#
def parse_results(metrics, architecture, nsembler, config):
    # Parse metrics to Pandas dataframe
    results = pandas.DataFrame.from_dict(metrics)
    results = results.transpose()
    results.columns = config["class_list"]
    # Backup to disk
    path_ensembler = os.path.join(config["path_results"], "phase_bagging" + "." + \
...
for architecture in architecture_list:
    ...
    for ensembler in config["ensembler_list"]:
        ...
        id, gt, pd, pd_prob = preprocessing(architecture, ensembler, config)
        # Compute metrics
        metrics = compute_metrics(gt, pd, pd_prob, config["class_list"])
        # Backup results
        metrics_df = parse_results(metrics, architecture, ensembler, config)
